...
const AdminLockProjectModal = ({ isOpen, onClose, onConfirm, project, loading }) => {
      ...
      <motion.div className='fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center px-4' initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
        <motion.div
          ...
          <div className='px-6 py-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between'>
            <div>
              <p className='text-xs uppercase tracking-wide text-gray-400 dark:text-gray-500'>Lock Project</p>
              <h3 className='text-xl font-semibold text-gray-900 dark:text-white'>{project.name}</h3>
            </div>
            <button onClick={onClose} className='text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 text-2xl leading-none' disabled={loading}>
              &times;
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className='px-6 py-6 space-y-5'>
            <div>
              ...
              <textarea
                ...
                placeholder='Explain why this project is being locked...'
                rows='3'
