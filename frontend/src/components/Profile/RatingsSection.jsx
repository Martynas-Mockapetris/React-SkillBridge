...
const RatingsSection = ({ ratings, stats, loading }) => {
  ...
  return (
    <div className='space-y-6'>
      {/* Rating Summary */}
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className='bg-gradient-to-br dark:from-light/5 dark:via-light/[0.02] from-primary/5 via-primary/[0.02] to-transparent p-6 rounded-lg border border-primary/10 dark:border-light/10 backdrop-blur-sm'>
      ...
      {ratings.ratings && ratings.ratings.length > 0 ? (
        <div className='space-y-4'>
          ...
          {ratings.ratings.map((rating, index) => (
            <motion.div key={rating._id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} className='bg-gradient-to-br dark:from-light/5 dark:via-light/[0.02] from-primary/5 via-primary/[0.02] to-transparent p-4 rounded-lg border border-primary/10 dark:border-light/10 backdrop-blur-sm'>
              {/* Reviewer Info */}
