    ...
    <motion.div
      ...
      <div className='flex items-center justify-between'>
        <div className='flex-1'>
          <h3 className='text-gray-600 dark:text-gray-300 text-sm font-medium tracking-wider uppercase'>{title}</h3>
      ...
      {trend && (
        <div className='mt-4 flex items-center'>
          ...
          <span className={`text-sm font-bold ${trend > 0 ? 'text-green-500' : 'text-red-500'}`}>{Math.abs(trend)}%</span>
          <span className='text-gray-600 dark:text-gray-300 text-sm ml-2'>vs last month</span>
        </div>
      )}
    </motion.div>
  )
